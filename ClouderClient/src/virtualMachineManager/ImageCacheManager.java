...
public class ImageCacheManager {
	...
	public static Image getFreeImageCopy(long imageId){
		Image vmi=lockFreeImageCopy(imageId);
		while(vmi==null){
	...
	private synchronized static Image lockFreeImageCopy(long imageId){
		loadImages();
		for(Image vmi:imageList)if(vmi.getId()==imageId&&vmi.getStatus()==VirtualMachineImageStatus.FREE){
			vmi.setStatus(VirtualMachineImageStatus.LOCK);
			return vmi;
		}
		return null;
	}
	public synchronized static void freeLockedImageCopy(Image vmi){
		vmi.setStatus(VirtualMachineImageStatus.FREE);
	}
	
	private synchronized static void addFreeImageCopy(Image vmi){
		imageList.add(vmi);
		saveImages();
	}
	...
	private static void loadImages(){
		if(imageList==null){
			...
			try(ObjectInputStream ois=new ObjectInputStream(new FileInputStream(imageListFile))){
				...
				System.out.println("Cargué "+imageList.size()+" imagenes");
				for(Image im:imageList){
					im.setStatus(VirtualMachineImageStatus.FREE);
				}
			} catch (IOException e) {
				e.printStackTrace();
