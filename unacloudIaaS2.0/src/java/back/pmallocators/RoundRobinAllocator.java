	...
	public void allocateVirtualMachines(List<VirtualMachineExecution> virtualMachineList,List<PhysicalMachine> physicalMachines,Map<Long,PhysicalMachineAllocationDescription> physicalMachineDescriptions) {
		...
		for(int nextVm=0,lastNextVm=0;nextVm<virtualMachineList.size();){
			for(PhysicalMachine pm:physicalMachines){
				...
				if(pmad==null&&nextVirtualMachine.getCores()<=pm.getCores()&&nextVirtualMachine.getRam()<=pm.getRam()){
					nextVirtualMachine.setExecutionNode(pm);
					pmad=new PhysicalMachineAllocationDescription(pm.getDatabaseId(),nextVirtualMachine.getCores(),nextVirtualMachine.getRam(),1);
					physicalMachineDescriptions.put(pm.getDatabaseId(),pmad);
					System.out.println("Asigne "+pm.getDatabaseId()+" no tenia vms");
					nextVm++;
				}else if(pmad.getVms()<MAX_VM_PER_PM&&pmad.getCores()+nextVirtualMachine.getCores()<=pm.getCores()&&pmad.getRam()+nextVirtualMachine.getRam()<=pm.getRam()){
					nextVirtualMachine.setExecutionNode(pm);
					pmad.addResources(nextVirtualMachine.getCores(),nextVirtualMachine.getRam(),1);
					nextVm++;
					System.out.println("Asigne "+pm.getDatabaseId()+" tenia vms");
				}else{
